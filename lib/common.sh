...
detect_system() {
  ...
  export OS_ID OS_VERSION_ID OS_PRETTY SSH_UNIT HAS_DOCKER HAS_UFW HAS_FAIL2BAN
}
...
load_config() {
  ...
  [[ -f "$cfg" ]] || { CONFIG_LOADED=0; return 0; }
  ...
  # NODE_NAME default = FQDN, cai para hostname curto.
  if [[ -z "${NODE_NAME:-}" ]]; then
    NODE_NAME="$(hostname -f 2>/dev/null || hostname 2>/dev/null || echo unknown)"
  fi
  export WEBHOOK_URL NODE_NAME ALERT_MIN_SEVERITY DIGEST_ENABLED \
